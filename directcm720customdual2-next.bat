...
rem --- Download the polite, verified downloader from the repo. -------------
rem This used to be a ~120-line base64 blob pasted into this .bat, which is
rem exactly why the "safer" version never ran: the blob still held the old
rem 16-connection script. Keep ONE copy - the one in the repo.
curl -fL -o "cm720-next.py" https://raw.githubusercontent.com/Bogyi2024/log/main/cm720-next.py
if errorlevel 1 (
    echo ERROR: Could not download cm720-next.py.
    exit /b 1
)
rem -------------------------------------------------------------------------
